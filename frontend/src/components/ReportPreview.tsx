...
export function ReportPreview({
  ...
  return (
    <div className="bg-white border border-hoban-border rounded-hoban-lg shadow-hoban-card p-5 print:shadow-none">
      {/* 헤더 */}
      <header className="flex items-start justify-between border-b-2 border-hoban-primary pb-3 mb-4">
        <div>
          <div className="text-[11px] uppercase tracking-wider text-hoban-primary font-bold">
            {tenantConfig.appName} · TBM 보고서
          </div>
          <h2 className="font-serif-display text-[20px] text-hoban-ink mt-0.5 leading-tight">
            {session.work_type_label || session.work_type || "제목 없음"}
          </h2>
          <div className="text-[11px] text-hoban-ink-soft mt-1">
            도메인: {session.domain ? DOMAIN_LABEL[session.domain] ?? session.domain : "(미지정)"}
            {" · "}
            {new Date(session.updated_at).toLocaleString("ko-KR")}
          </div>
        </div>
        <div className="text-[10px] text-hoban-ink-mute font-mono shrink-0 max-w-[140px] truncate text-right">
          {session.session_id}
        </div>
      </header>
      ...
      {finalSummary && (
        <section className="mb-4">
          <h3 className="text-[11px] uppercase tracking-wider text-hoban-primary font-bold mb-1">
            최종 요약
          </h3>
          <p className="text-sm text-hoban-ink whitespace-pre-wrap leading-relaxed bg-hoban-primary-wash border-l-4 border-hoban-primary px-3 py-2">
            {finalSummary}
          </p>
        </section>
      )}
      ...
      <section className="mb-4">
        ...
        <ul className="text-sm text-hoban-ink space-y-0.5">
          ...
          {Object.keys(session.prior_info ?? {}).length === 0 && (
            <li className="text-hoban-ink-mute italic">(미입력)</li>
          )}
        </ul>
      </section>
        ...
        return (
          <section className="mb-4">
            <h3 className="text-[11px] uppercase tracking-wider text-hoban-primary font-bold mb-1">
              ...
              {skipped.length > 0 && (
                <span className="ml-1 text-hoban-ink-mute font-normal normal-case tracking-normal">
                  · 건너뜀 {skipped.length}
                </span>
              )}
            </h3>
            {items.length === 0 ? (
              <p className="text-xs text-hoban-ink-mute italic">항목 없음</p>
            ) : (
              <>
                {completed.length > 0 && (
                  <ul className="text-sm text-hoban-ink space-y-0.5">
                ...
                {skipped.length > 0 && (
                  <ul className="text-sm text-hoban-ink-mute italic space-y-0.5 mt-1.5">
                    {skipped.map((c, i) => (
                ...
                {incomplete.length > 0 && (
                  <ul className="text-xs text-hoban-ink-mute space-y-0.5 mt-1.5">
                    {incomplete.map((c, i) => (
      ...
      <section className="mb-4">
        ...
        {attendees.length === 0 ? (
          <p className="text-xs text-hoban-ink-mute italic">참석자 미입력</p>
        ) : (
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {attendees.map((a) => (
              <li
                key={a.id}
                className="flex items-center gap-2 border border-hoban-border rounded-hoban px-2 py-1.5 bg-hoban-bg-card"
              >
                <span
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold text-hoban-ink truncate">
                    {a.name}
                    {a.role && (
                      <span className="ml-1 text-[10px] text-hoban-ink-soft font-normal">
                        {a.role}
                      </span>
                    )}
                  </div>
                  <div className="text-[10px] text-hoban-ink-mute">
                    {a.signed
                ...
                {a.signature_data_url && (
                  <img
                    ...
                    alt={`${a.name} 서명`}
                    className="shrink-0 h-7 w-auto max-w-[64px] bg-white border border-hoban-border rounded-hoban"
                  />
                )}
              </li>
            ))}
      ...
      {allCitations.length > 0 && (
        <section className="mb-2">
          ...
          <ul className="text-[11px] text-hoban-ink-soft space-y-0.5">
      ...
      <footer className="text-[10px] text-hoban-ink-mute pt-3 mt-3 border-t border-hoban-border">
        {tenantConfig.appName} · {tenantConfig.companyName} Industrial Safety · 클라이언트 PDF 생성(오프라인)
      </footer>
    </div>
  );
}
...
function Field({
  ...
  if (!value || (Array.isArray(value) && value.length === 0)) {
    return (
      <div className="mb-2">
        <div className="text-[10px] uppercase tracking-wider text-hoban-ink-mute font-bold">
          {label}
        </div>
        <div className="text-xs text-hoban-ink-mute italic">—</div>
      </div>
    );
  }
  return (
    <div className="mb-2">
      ...
      {Array.isArray(value) ? (
        <ul className="text-sm text-hoban-ink">
          {value.map((v, i) => (
            <li key={i}>· {v}</li>
          ))}
        </ul>
      ) : (
        <p className="text-sm text-hoban-ink whitespace-pre-wrap leading-snug">{value}</p>
      )}
    </div>
  );
}
