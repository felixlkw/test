    ...
    (event: unknown) => {
      ...
      switch (e.type) {
        ...
        case "response.output_text.delta":
          recordFirstToken();
          return;
        // 2026-05-23 hotfix — Realtime GA가 ["audio","text"] 콤보 거부. 단일 audio
        // 모달리티로 통일. 따라서 text.done 이벤트는 더 이상 도착하지 않으며 dedup
        // 로직 불필요. EHS 긴 답변 가독성은 LLM이 transcript 안에서 \n으로 라인 구분
        // 하고 프론트가 whitespace-pre-wrap으로 렌더하는 방식으로 해결(prompt.py).
